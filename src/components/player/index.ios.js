...
const PlayerScreen = (props) => {
  ...
  useEffect(() => {
    ...
    // ) {
    //   if (parseInt(position) === parseInt(duration)) {
    //     console.log('song ended!!!');
    //     next();
    //   }
    // }
  }, [duration, position, playbackState]);
  const returnPlayBtn = () => {
  ...
  const getTrackDetail = async () => {
    ...
    let track = await TrackPlayer.getTrack(String(currenttrackid));
    if (
      ...
      props.nextadd({data: props.Player, id: track.id});
      // setrackid(track.id);
      // settitle(track.title);
      // setartist(track.artist);
      // console.log(currenttrackid, track);
    }
  };
  useEffect(() => {
    getTrackDetail();
  ...
  const hidecurrentsong = async () => {
    ...
    if (likeresults && likeresults.success) {
      ...
      await props.updatecurrentplaylist({trackid: trackid, data: props.Player});
      await next();
      await TrackPlayer.remove(String(trackid));
  ...
  const loadImages = (card) => {
    return (
      <ImageBackground
        style={styles.image}
        source={
          {uri: 'https://mptone.com/media/placeholder/icon_placeholder.jpeg'}
        ...
        resizeMode="cover">
        <LinearGradient
          colors={['rgba(38, 38, 38, 0)', 'rgba(38, 38, 38, 0)', '#262626']}
    ...
    <View style={[styles.container]}>
      <View style={styles.viewstyle1}>
        <Swiper
          ...
          renderCard={(card) => {
            return <View style={styles.slide1}>{loadImages(card)}</View>;
          }}
          onSwiped={(cardIndex) => {}}
          onSwipedAll={() => {
