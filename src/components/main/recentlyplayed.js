...
const RecentlyPlayed = (props) => {
  ...
  const renderItem = ({item}) => (
    <>
      {!item.hide ? (
        <TouchableOpacity
          key={`index_3_recent${String(item.id)}`}
  ...
  return (
    <View>
      {props.recentplayed && props.recentplayed.length > 0 ? (
        <View style={styles.liststyle}>
          <View style={styles.spacingtop}>
            <Text style={styles.fontstyle}>{t('Recently_played')}</Text>
          </View>
          <FlatList
