...
import {EMAIL_SCREEN} from '../../../navigation/screen';
    ...
    if (username && String(username).trim()) {
      if (username.length < 3) {
        ...
        if (verify && verify.success) {
          await props.setValue({username: username});
          setMessage('');
          setStatus(true);
          PushScreen(EMAIL_SCREEN, props);
        } else {
          ...
          <Form>
            ...
            <View style={styles.padderTop}>
              <Text adjustsFontSizeToFit={true} style={styles.fontStyle}>
                {t(
                  'This_appears_on_your_profile_and_an_alternative_for_logging_in',
                )}
              </Text>
