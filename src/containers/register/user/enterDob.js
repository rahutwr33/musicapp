...
import {GENDER_SCREEN} from '../../../navigation/screen';
import moment from 'moment';
import {useTranslation} from 'react-i18next';

const DOBScreen = (props) => {
  let [year, setYear] = useState(props.user.date_of_birth.split('-')[0]);
  let [day, setDay] = useState(props.user.date_of_birth.split('-')[1]);
  let [month, setMonth] = useState(props.user.date_of_birth.split('-')[2]);
  ...
  const submit = async () => {
    if (year && day && month) {
      ...
      if (isUTC) {
        await props.setValue({date_of_birth: `${year}-${month}-${day}`});
        setMessage('');
        setStatus(true);
        PushScreen(GENDER_SCREEN, props);
      } else {
      ...
      <Container style={styles.container}>
        ...
        <Content
          ...
          <Form>
            ...
            <Button onPress={() => submit()} buttonTitle="Next" />
          </Form>
        </Content>
      </Container>
    </KeyboardAvoid>
